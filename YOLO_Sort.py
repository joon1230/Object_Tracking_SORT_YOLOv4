...
out = cv2.VideoWriter( os.path.join( save_path, "res_Yolo_Sort.avi") , fourcc, int(fps), (int(image_w), int(image_h)))
...
while cap.isOpened():
    ...
    if cnt < 2000:
        ...
        # tracking
        trackers = sort_tracker.update(obj[is_in])
        for d in trackers:
            d = d.astype(np.int32)
            ...
            cv2.rectangle( frame , p1 , p2 , cls_color.get(int(d[5]), (255,204,92) ) , 3 )
            cv2.putText( frame , f"{cls_dict[int(d[5])]}_{d[4]}" , p1 , cv2.FONT_HERSHEY_DUPLEX , 1, (0,0,0) )
